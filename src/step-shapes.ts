...
const AI_ACTION_EMAIL: StepShape = {
    ...
    notes: [
        'Always set `pipelineStepPrompt.type: "email"`.',
        'Always set `onApproval.action: "schedule-email"` — this is what triggers actual sending.',
        'Always set `next.conditions.approvalRequired: true` — blocks the workflow until the user reviews.',
        'For outreach from a user-selected connected mailbox, add an `outreachProfile` input page to context.inputPages with fields: name, fromEmailLabel, fromEmail (type: connected_emails_selector_multiple), replyToEmail.',
    ...
    example: {
        ...
        configuration: {
            ...
            fields: [
                ...
                { name: 'signature', label: 'Email signature', type: 'rich_text' },
            ],
        },
    },
    notes: [
